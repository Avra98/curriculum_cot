...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--wandb_group", type=str, default="baseline_multi_output_pipeline_resume")
...
def choose_output_root(args: argparse.Namespace, checkpoint_root: Path) -> Path:
    if str(args.output_root).strip():
        requested_root = Path(normalize_to_final_checkpoint_root(args.output_root, "multi_output_cell_policy")).resolve()
    ...
    run_tag = str(args.run_tag).strip() or time.strftime("%Y%m%d_%H%M%S")
    return checkpoint_root / run_tag / f"baseline_pipeline_{args.total_empties_hint}empty_{args.max_stage}stage_resume"
    ...
    *,
    ...
    if args.use_wandb:
        ...
        command.extend(
            [
                "--wandb_project",
                "sudoku-multi-output-sft",
                "--wandb_run_name",
                f"baseline_stage{stage:02d}_sft_i{stage}_{args.total_empties_hint}empty",
                "--wandb_mode",
                args.wandb_mode,
            ]
        )
    ...
    *,
    ...
    if args.use_wandb:
        ...
        command.extend(
            [
                "--wandb_project",
                "sudoku-multi-output-grpo",
                "--wandb_run_name",
                f"baseline_stage{stage:02d}_grpo_i{stage}_{args.total_empties_hint}empty",
                "--wandb_mode",
                args.wandb_mode,
            ]
        )
...
def main() -> None:
    args = parse_args()
    checkpoint_root = Path(normalize_to_final_checkpoint_root(args.checkpoint_root, "multi_output_cell_policy")).resolve()
