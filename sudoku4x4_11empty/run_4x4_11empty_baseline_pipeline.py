...
DEFAULT_CHECKPOINT_ROOT = Path(final_checkpoint_root("sudoku4x4_11empty", "baseline"))
DEFAULT_CACHE_DIR = PARENT_DIR / ".hf_cache"
DEFAULT_MODEL_NAME = "Qwen/Qwen2.5-0.5B-Instruct"
DEFAULT_WANDB_GROUP = "sudoku4x4_11empty_baseline_pipeline"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--grpo_limit_train_examples", type=int, default=0)
...
def choose_output_root(args: argparse.Namespace, checkpoint_root: Path) -> Path:
    if str(args.output_root).strip():
        requested_root = Path(
            normalize_to_final_checkpoint_root(args.output_root, "sudoku4x4_11empty", "baseline")
        ).resolve()
    ...
    run_tag = str(args.run_tag).strip() or time.strftime("%Y%m%d_%H%M%S")
    return checkpoint_root / run_tag / f"baseline_pipeline_{args.total_empties_hint}empty_{args.max_stage}stage_4x4"
    ...
    if args.use_wandb:
        ...
        command.extend(
            [
                ...
                "--wandb_run_name",
                f"sudoku4x4_baseline_stage{stage:02d}_sft_i{stage}_{args.total_empties_hint}empty",
                "--wandb_mode",
                args.wandb_mode,
            ]
        )
    ...
    if args.use_wandb:
        ...
        command.extend(
            [
                ...
                "--wandb_run_name",
                f"sudoku4x4_baseline_stage{stage:02d}_grpo_i{stage}_{args.total_empties_hint}empty",
                "--wandb_mode",
                args.wandb_mode,
            ]
        )
...
def main() -> None:
    args = parse_args()
    checkpoint_root = Path(
        normalize_to_final_checkpoint_root(args.checkpoint_root, "sudoku4x4_11empty", "baseline")
    ).resolve()
