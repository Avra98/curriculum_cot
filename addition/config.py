...
class ExperimentConfig:
    ...
    def discrete_vocab_size(self) -> int:
        return 13 + self.eval_max_digits

    @property
    def base_sequence_length(self) -> int:
        return (self.eval_max_digits * 2) + 4
...
def add_config_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--carry_loss_weight", type=float, default=0.5)
    ...
    parser.add_argument("--stage_accuracy_threshold", type=float, default=0.98)
...
def build_config_from_args(args: argparse.Namespace) -> ExperimentConfig:
    ...
    if args.wandb_mode == "disabled":
        use_wandb = False
    output_dir = args.output_dir or str(default_output_root() / f"{args.model}_seed{args.seed}")
